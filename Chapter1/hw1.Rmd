...
nrow(result)
```
54 patients came to the office.

```{r}
(waited_patients = sum(result$wait_time > 0))
```
16 patients had to wait for a doctor.

```{r}
sum(result$wait_time[result$wait_time > 0]) / waited_patients
```
Their average wait was 2.64 minutes.

```{r}
result$out_time[nrow(result)]
```
Officed closed after 475.80 minutes since opening. Approximately 3:54pm. 
